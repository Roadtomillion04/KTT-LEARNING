...
// Well nvm, now json link is changed or removed, the site is up tho


// finding model of a device
// https://stackoverflow.com/a/69225035
// so they say it's scraping from Applewikit page, as it's scraping we will need internet
...
//    
//    guard identifier != unrecognized,
//          let url = URL(string: "https://api.appledb.dev/device/\(identifier).json") else {
...
//                completion(name) // Restituisce il nome dell'iPhone, es. "iPhone 16e"
//            } else {
...
// Since the online fetching is outdated(not working)
// for now I use solution from same user above, manually adding everything in Enum
// and also yeah it needs to be updated every time new ios device come out
