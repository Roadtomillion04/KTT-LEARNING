...
class TripsViewModel: ObservableObject {
    ...
    func totalAdvances(_ tripsResult: [APIService.TripsDataModel.Results]) -> String {
    ...
    func totalExpenses(_ tripsResult: [APIService.TripsDataModel.Results]) -> String {
    ...
    func onAppear(apiService: APIService) async {
        ...
        do {
            try await apiService.getTripsData(startDate: startDate.toString(), endDate: endDate.toString())
        } catch {
    ...
    func onReload(apiService: APIService) async {
        // okay isLoading is the root cause
    ...
    func calculateAdvance(_ tripAdvance: [APIService.TripsDataModel.TripAdvance]) -> String {
        
        return tripAdvance.map( { $0.totalAmount?.toDouble() ?? 0 } ).reduce(0, +).round()
        
    }
    
    func approvedExpense(_ tripExpense: [APIService.TripsDataModel.TripExpense]) -> String {
        
        return tripExpense.filter( { $0.status == 1 } ).map( { $0.amount?.toDouble() ?? 0 } ).reduce(0, +).round()
    }
    
    func pendingExpense(_ tripExpense: [APIService.TripsDataModel.TripExpense]) -> String {
        
        return tripExpense.filter( { $0.status == 0 } ).map( { $0.amount?.toDouble() ?? 0 } ).reduce(0, +).round()
    }
    
    func rejectedExpense(_ tripExpense: [APIService.TripsDataModel.TripExpense]) -> String {
